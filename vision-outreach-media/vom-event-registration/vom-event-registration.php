<?php
/**
 * Plugin Name: Vision Outreach Event Registration
 * Description: Bilingual (EN/NL) event registration & sign-up forms for churches and ministries. One engine, several ready-made form types — potluck/picnic sign-up, event RSVP, volunteer/serving sign-up, and workshop registration. Drop any of them on a page with a shortcode; responses are stored in your WordPress database and exportable to CSV. Shortcode: [event_registration event="picnic"].
 * Version: 2.0.0
 * Author: Vision Outreach Media
 * License: GPL-2.0-or-later
 * Text Domain: vom-evreg
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'VOM_EVREG_VER', '2.0.0' );
define( 'VOM_EVREG_PAGE_TITLE', 'Picknick' );
define( 'VOM_EVREG_PAGE_SLUG', 'picknick' );

function vom_evreg_table() {
	global $wpdb;
	return $wpdb->prefix . 'picnic_signups';
}

/* =========================================================
   PRESETS — each is one type of registration form.
   Add your own by copying a block and changing the fields.
   ========================================================= */
function vom_evreg_cat( $k, $e, $en, $eni, $nl, $nli ) {
	return array( 'key' => $k, 'emoji' => $e, 'en' => array( 'name' => $en, 'ideas' => $eni ), 'nl' => array( 'name' => $nl, 'ideas' => $nli ) );
}

function vom_evreg_presets() {
	$presets = array();

	/* ---- 1. PICNIC / POTLUCK ---- */
	$presets['picnic'] = array(
		'label'         => 'Picnic / Potluck',
		'hasParty'      => true,
		'hasSelections' => true,
		'allowCustom'   => true,
		'supplyKey'     => 'supplies',
		'categories'    => array(
			vom_evreg_cat( 'mains', "\xF0\x9F\x8D\x97", 'Mains & Grill', array( 'BBQ chicken', 'Sausages / hot dogs', 'Burgers', 'Rice dish', 'Veggie skewers' ), 'Hoofdgerechten', array( 'BBQ-kip', 'Worstjes / hotdogs', 'Hamburgers', 'Rijstgerecht', 'Groentespiesjes' ) ),
			vom_evreg_cat( 'sides', "\xF0\x9F\xA5\x97", 'Salads & Sides', array( 'Green salad', 'Potato salad', 'Pasta salad', 'Bread rolls', 'Chips & dip' ), 'Salades & Bijgerechten', array( 'Groene salade', 'Aardappelsalade', 'Pastasalade', 'Broodjes', 'Chips & dip' ) ),
			vom_evreg_cat( 'fruit', "\xF0\x9F\x8D\x89", 'Fruit', array( 'Watermelon', 'Fruit salad', 'Grapes', 'Berries' ), 'Fruit', array( 'Watermeloen', 'Fruitsalade', 'Druiven', 'Bessen' ) ),
			vom_evreg_cat( 'desserts', "\xF0\x9F\x8D\xB0", 'Desserts', array( 'Cake', 'Brownies', 'Cookies', 'Fruit tart' ), 'Nagerechten', array( 'Taart', 'Brownies', 'Koekjes', 'Vruchtentaart' ) ),
			vom_evreg_cat( 'drinks', "\xF0\x9F\xA5\xA4", 'Drinks', array( 'Water', 'Lemonade', 'Iced tea', 'Juice', 'Coffee & tea' ), 'Drankjes', array( 'Water', 'Limonade', 'IJsthee', 'Sap', 'Koffie & thee' ) ),
			vom_evreg_cat( 'supplies', "\xF0\x9F\xA7\xBA", 'Supplies', array( 'Plates', 'Cups', 'Napkins', 'Cutlery', 'Trash bags', 'Cooler with ice' ), 'Benodigdheden', array( 'Bordjes', 'Bekers', 'Servetten', 'Bestek', 'Vuilniszakken', 'Koelbox met ijs' ) ),
		),
		'overrides'     => array( 'en' => array(), 'nl' => array() ),
	);

	/* ---- 2. EVENT RSVP (no items) ---- */
	$presets['rsvp'] = array(
		'label'         => 'Event RSVP',
		'hasParty'      => true,
		'hasSelections' => false,
		...
		'overrides'     => array(
			'en' => array(
				'kicker' => "You're invited", 'title' => 'Event RSVP',
				'blurb' => 'Please let us know if you can join us — it helps us plan. Add your name and how many are coming.',
				'formTitle' => "\xF0\x9F\x93\x9D RSVP", 'formLead' => 'Coming as a family or group? Add everyone in one go with the headcount.',
				'stPeople' => 'People attending', 'submit' => 'Confirm my RSVP',
				'notesLabel' => 'Notes / message', 'notesExtra' => '(optional — dietary needs, questions)', 'notesPh' => 'e.g. 2 vegetarians, arriving a little late',
				'listTitle' => "\xF0\x9F\x8E\x9F\xEF\xB8\x8F Who's coming", 'empty' => 'No RSVPs yet — be the first!', 'tAdded' => 'RSVP received — thank you!',
			),
			'nl' => array(
				'kicker' => 'Je bent uitgenodigd', 'title' => 'Aanmelden',
				'blurb' => 'Laat ons weten of je erbij kunt zijn — dat helpt ons met plannen. Vul je naam in en met hoeveel personen je komt.',
				'formTitle' => "\xF0\x9F\x93\x9D Aanmelden", 'formLead' => 'Kom je met je gezin of groep? Meld iedereen in één keer aan met het aantal personen.',
				'stPeople' => 'Aantal aanwezig', 'submit' => 'Bevestig mijn aanmelding',
				'notesLabel' => 'Opmerking', 'notesExtra' => '(optioneel — dieetwensen, vragen)', 'notesPh' => 'bijv. 2 vegetariërs, iets later',
				'listTitle' => "\xF0\x9F\x8E\x9F\xEF\xB8\x8F Wie komt er", 'empty' => 'Nog geen aanmeldingen — wees de eerste!', 'tAdded' => 'Aanmelding ontvangen — bedankt!',
			),
		),
	);

	/* ---- 3. VOLUNTEER / SERVING SIGN-UP ---- */
	$presets['volunteer'] = array(
		'label'         => 'Volunteer / Serving',
		'hasParty'      => false,
		'hasSelections' => true,
		'allowCustom'   => false,
		'supplyKey'     => null,
		'categories'    => array(
			vom_evreg_cat( 'setup', "\xF0\x9F\x94\xA7", 'Setup & Cleanup', array( 'Arrive early to set up', 'Chairs & tables', 'Stay to clean up', 'Take out trash' ), 'Opbouw & Opruimen', array( 'Vroeg komen opbouwen', 'Stoelen & tafels', 'Blijven opruimen', 'Afval wegbrengen' ) ),
			vom_evreg_cat( 'kitchen', "\xE2\x98\x95", 'Kitchen & Hospitality', array( 'Make coffee & tea', 'Bring snacks', 'Serve food', 'Wash dishes' ), 'Keuken & Gastvrijheid', array( 'Koffie & thee zetten', 'Iets lekkers meenemen', 'Eten serveren', 'Afwassen' ) ),
			vom_evreg_cat( 'welcome', "\xF0\x9F\xA4\x9D", 'Welcome & Care', array( 'Greet at the door', 'Hand out programs', 'Parking help', "Children's ministry" ), 'Welkom & Zorg', array( 'Verwelkomen bij de deur', "Programma's uitdelen", 'Parkeerhulp', 'Kinderdienst' ) ),
			vom_evreg_cat( 'program', "\xF0\x9F\x8E\xA4", 'Service & Tech', array( 'Worship / music', 'Bible reading', 'Sound & tech', 'Photography' ), 'Dienst & Techniek', array( 'Aanbidding / muziek', 'Bijbellezing', 'Geluid & techniek', 'Fotografie' ) ),
		),
		'overrides'     => array(
			'en' => array(
				'kicker' => 'Lend a hand', 'title' => 'Volunteer Sign-Up',
				'blurb' => 'Many hands make light work! Choose where you can help and add your name — thank you for serving.',
				'formTitle' => "\xF0\x9F\x99\x8C Sign up to help", 'formLead' => 'Add your name and pick the areas you can help with.',
				'stPeople' => 'People helping', 'stDishes' => 'Roles filled', 'stCats' => 'Areas covered',
				'bringLabel' => 'Where can you help?', 'bringOpt' => '', 'bringHelp' => 'Tap the roles you can take. Pick as many as you like.',
				'trayTitle' => "You'll help with", 'basketHint' => 'Nothing selected yet. Tap a role above.',
				'submit' => 'Sign me up', 'notesLabel' => 'Notes', 'notesExtra' => '(optional)', 'notesPh' => 'e.g. I can only help before 12:00',
				'listTitle' => "\xF0\x9F\x99\x8C Who's helping", 'empty' => 'No sign-ups yet — be the first!', 'comingAlong' => "\xF0\x9F\x99\x8B Signed up (no role yet)",
				'tAdded' => 'Thank you for helping!', 'neededPrefix' => 'Still need help with:', 'neededSuffix' => '— tap to sign up',
			),
			'nl' => array(
				'kicker' => 'Steek de handen uit de mouwen', 'title' => 'Vrijwilligers Aanmelden',
				'blurb' => 'Vele handen maken licht werk! Kies waar je kunt helpen en vul je naam in — bedankt voor je inzet.',
				'formTitle' => "\xF0\x9F\x99\x8C Meld je aan om te helpen", 'formLead' => 'Vul je naam in en kies waar je kunt helpen.',
				'stPeople' => 'Aantal helpers', 'stDishes' => 'Taken ingevuld', 'stCats' => 'Gebieden gedekt',
				'bringLabel' => 'Waar kun je helpen?', 'bringOpt' => '', 'bringHelp' => 'Tik op de taken die je kunt doen. Kies er zoveel je wilt.',
				'trayTitle' => 'Jij helpt met', 'basketHint' => 'Nog niets gekozen. Tik hierboven op een taak.',
				'submit' => 'Meld mij aan', 'notesLabel' => 'Opmerking', 'notesExtra' => '(optioneel)', 'notesPh' => 'bijv. ik kan alleen voor 12:00',
				'listTitle' => "\xF0\x9F\x99\x8C Wie helpt er", 'empty' => 'Nog geen aanmeldingen — wees de eerste!', 'comingAlong' => "\xF0\x9F\x99\x8B Aangemeld (nog geen taak)",
				'tAdded' => 'Bedankt voor je hulp!', 'neededPrefix' => 'Nog hulp nodig bij:', 'neededSuffix' => '— tik om aan te melden',
			),
		),
	);

	/* ---- 4. WORKSHOP / COURSE REGISTRATION ---- */
	$presets['workshop'] = array(
		...
		'supplyKey'     => null,
		'categories'    => array(
			vom_evreg_cat( 'foundations', "\xF0\x9F\x93\x96", 'Foundations', array( 'Intro to the Bible', 'Prayer basics', 'New believers' ), 'Fundamenten', array( 'Kennismaking met de Bijbel', 'Basis van gebed', 'Nieuwe gelovigen' ) ),
			vom_evreg_cat( 'family', "\xF0\x9F\x91\xAA", 'Family & Relationships', array( 'Marriage', 'Parenting', 'Singles' ), 'Gezin & Relaties', array( 'Huwelijk', 'Opvoeding', 'Singles' ) ),
			vom_evreg_cat( 'growth', "\xF0\x9F\x8C\xB1", 'Growth & Service', array( 'Discipleship', 'Worship team', 'Outreach & evangelism' ), 'Groei & Dienst', array( 'Discipelschap', 'Aanbiddingsteam', 'Evangelisatie' ) ),
			vom_evreg_cat( 'youth', "\xF0\x9F\xA7\x92", 'Youth & Kids', array( 'Youth track', 'Kids program' ), 'Jeugd & Kids', array( 'Jeugdtrack', 'Kinderprogramma' ) ),
		),
		'overrides'     => array(
			'en' => array(
				'kicker' => 'Registration open', 'title' => 'Workshop Registration',
				'blurb' => "Reserve your spot. Add your name, how many are attending, and choose the sessions you'd like to join.",
				'formTitle' => "\xF0\x9F\x93\x9D Register", 'formLead' => 'Registering for your family or group? Add the total number attending.',
				'stPeople' => 'People registered', 'stDishes' => 'Seats reserved', 'stCats' => 'Tracks chosen',
				'bringLabel' => 'Choose your session(s)', 'bringOpt' => '', 'bringHelp' => "Tap the sessions you'd like to attend. Choose as many as you like.",
				'trayTitle' => 'Your sessions', 'basketHint' => 'No sessions chosen yet. Tap one above.',
				'submit' => 'Complete registration', 'notesLabel' => 'Notes', 'notesExtra' => '(optional — questions, needs)', 'notesPh' => 'e.g. wheelchair access needed',
				'listTitle' => "\xF0\x9F\x93\x8B Who's registered", 'empty' => 'No registrations yet — be the first!', 'comingAlong' => "\xF0\x9F\x99\x8B Registered (no session yet)",
				'tAdded' => "You're registered — see you there!", 'neededPrefix' => 'Open tracks:', 'neededSuffix' => '— tap to join',
			),
			'nl' => array(
				'kicker' => 'Inschrijving open', 'title' => 'Workshop Inschrijving',
				'blurb' => 'Reserveer je plek. Vul je naam in, met hoeveel personen je komt, en kies de sessies die je wilt volgen.',
				'formTitle' => "\xF0\x9F\x93\x9D Inschrijven", 'formLead' => 'Schrijf je je gezin of groep in? Vul het totale aantal deelnemers in.',
				'stPeople' => 'Ingeschreven personen', 'stDishes' => 'Plaatsen gereserveerd', 'stCats' => 'Tracks gekozen',
				'bringLabel' => 'Kies je sessie(s)', 'bringOpt' => '', 'bringHelp' => 'Tik op de sessies die je wilt volgen. Kies er zoveel je wilt.',
				'trayTitle' => 'Jouw sessies', 'basketHint' => 'Nog geen sessies gekozen. Tik er hierboven een.',
				'submit' => 'Inschrijving afronden', 'notesLabel' => 'Opmerking', 'notesExtra' => '(optioneel — vragen, wensen)', 'notesPh' => 'bijv. rolstoeltoegang nodig',
				'listTitle' => "\xF0\x9F\x93\x8B Wie is ingeschreven", 'empty' => 'Nog geen inschrijvingen — wees de eerste!', 'comingAlong' => "\xF0\x9F\x99\x8B Ingeschreven (nog geen sessie)",
				'tAdded' => 'Je bent ingeschreven — tot dan!', 'neededPrefix' => 'Open tracks:', 'neededSuffix' => '— tik om deel te nemen',
			),
		),
	);

	return apply_filters( 'vom_evreg_presets', $presets );
}

function vom_evreg_get_preset( $key ) {
	$all = vom_evreg_presets();
	if ( isset( $all[ $key ] ) ) { return $all[ $key ]; }
	return $all['picnic'];
}

/* ---------- activation: create/upgrade table ---------- */
register_activation_hook( __FILE__, 'vom_evreg_activate' );
function vom_evreg_activate() {
	global $wpdb;
	$table   = vom_evreg_table();
	$charset = $wpdb->get_charset_collate();
	require_once ABSPATH . 'wp-admin/includes/upgrade.php';
	$sql = "CREATE TABLE $table (
		id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
		event VARCHAR(40) NOT NULL DEFAULT 'picnic',
	...
	dbDelta( $sql );
	// Backfill any rows from before the event column existed.
	$wpdb->query( "UPDATE $table SET event='picnic' WHERE event IS NULL OR event=''" );
	vom_evreg_ensure_page();
}

/* ---------- auto-create the picnic page ---------- */
function vom_evreg_ensure_page() {
	$existing = (int) get_option( 'vom_picnic_page_id' );
	if ( $existing && get_post_status( $existing ) === 'publish' ) { return $existing; }
	$by_slug = get_page_by_path( VOM_EVREG_PAGE_SLUG );
	if ( $by_slug ) { update_option( 'vom_picnic_page_id', (int) $by_slug->ID ); return (int) $by_slug->ID; }
	$id = wp_insert_post( array(
		'post_title'   => VOM_EVREG_PAGE_TITLE,
		'post_name'    => VOM_EVREG_PAGE_SLUG,
		'post_status'  => 'publish',
		'post_type'    => 'page',
		'post_content' => '[event_registration event="picnic"]',
	) );
	if ( $id && ! is_wp_error( $id ) ) {
		update_option( 'vom_picnic_page_id', (int) $id );
		set_transient( 'vom_evreg_just_created', 1, 60 );
		return (int) $id;
	}
	return 0;
}
add_action( 'admin_init', 'vom_evreg_ensure_page' );

add_action( 'admin_notices', function () {
	if ( ! current_user_can( 'manage_options' ) || ! get_transient( 'vom_evreg_just_created' ) ) { return; }
	delete_transient( 'vom_evreg_just_created' );
	$pid = (int) get_option( 'vom_picnic_page_id' );
	if ( ! $pid ) { return; }
	$url = get_permalink( $pid );
	echo '<div class="notice notice-success is-dismissible"><p>&#127881; Registration page is live: <a href="' . esc_url( $url ) . '" target="_blank"><strong>' . esc_html( $url ) . '</strong></a></p></div>';
} );

/* ---------- data helpers ---------- */
function vom_evreg_parse_items( $raw ) {
	$out = array();
...
function vom_evreg_state( $event ) {
	global $wpdb;
	$table = vom_evreg_table();
	$event = $event ? sanitize_key( $event ) : 'picnic';
	$rows  = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $table WHERE event = %s ORDER BY id ASC", $event ), ARRAY_A );
	$items = array();
	$people = 0; $households = 0; $dishes = 0; $cats = array();
	if ( $rows ) {
		foreach ( $rows as $r ) {
			$its = vom_evreg_parse_items( $r['items'] );
			$items[] = array(
				'id'        => (string) $r['id'],
				'name'      => $r['name'],
...
add_action( 'rest_api_init', function () {
	register_rest_route( 'picnic/v1', '/state', array( 'methods' => 'GET', 'callback' => 'vom_evreg_rest_state', 'permission_callback' => '__return_true' ) );
	register_rest_route( 'picnic/v1', '/signup', array( 'methods' => 'POST', 'callback' => 'vom_evreg_rest_signup', 'permission_callback' => '__return_true' ) );
	register_rest_route( 'picnic/v1', '/remove', array( 'methods' => 'POST', 'callback' => 'vom_evreg_rest_remove', 'permission_callback' => '__return_true' ) );
} );

function vom_evreg_rest_state( WP_REST_Request $req ) {
	$event = sanitize_key( $req->get_param( 'event' ) );
	return rest_ensure_response( vom_evreg_state( $event ? $event : 'picnic' ) );
}

function vom_evreg_rest_signup( WP_REST_Request $req ) {
	global $wpdb;
	$body  = $req->get_json_params();
	$event = isset( $body['event'] ) ? sanitize_key( $body['event'] ) : 'picnic';
	if ( $event === '' ) { $event = 'picnic'; }
	...
	if ( $name === '' ) { return new WP_Error( 'invalid', 'Name is required.', array( 'status' => 400 ) ); }
	if ( $party < 1 ) { $party = 1; }
	$clean = array();
	if ( isset( $body['items'] ) && is_array( $body['items'] ) ) {
		...
		foreach ( $body['items'] as $it ) {
			...
			if ( $dish === '' ) { continue; }
			$clean[] = array( 'id' => uniqid( 'i', true ), 'category' => isset( $it['category'] ) ? sanitize_key( $it['category'] ) : '', 'dish' => mb_substr( $dish, 0, 120 ) );
		}
	}
	$wpdb->insert( vom_evreg_table(), array(
	...
	return rest_ensure_response( vom_evreg_state( $event ) );
}

function vom_evreg_rest_remove( WP_REST_Request $req ) {
	global $wpdb;
	$table  = vom_evreg_table();
	$body   = $req->get_json_params();
	$event  = isset( $body['event'] ) ? sanitize_key( $body['event'] ) : 'picnic';
	...
	if ( $pid < 1 ) { return new WP_Error( 'invalid', 'Missing id.', array( 'status' => 400 ) ); }
	if ( $itemId === '' ) {
		$wpdb->delete( $table, array( 'id' => $pid ) );
	} else {
		$raw  = $wpdb->get_var( $wpdb->prepare( "SELECT items FROM $table WHERE id = %d", $pid ) );
		$kept = array();
		foreach ( vom_evreg_parse_items( $raw ) as $it ) {
...
/* ---------- shortcode ---------- */
add_shortcode( 'event_registration', 'vom_evreg_shortcode' );
add_shortcode( 'picnic_signup', function ( $atts ) { return vom_evreg_shortcode( array( 'event' => 'picnic' ) ); } );

function vom_evreg_shortcode( $atts ) {
	$atts   = shortcode_atts( array( 'event' => 'picnic' ), $atts, 'event_registration' );
	$event  = sanitize_key( $atts['event'] );
	$preset = vom_evreg_get_preset( $event );
	$cfg = array(
		'rest'          => esc_url_raw( rest_url( 'picnic/v1/' ) ),
		'nonce'         => wp_create_nonce( 'wp_rest' ),
		'event'         => $event,
		'hasParty'      => (bool) $preset['hasParty'],
		'hasSelections' => (bool) $preset['hasSelections'],
		'allowCustom'   => (bool) $preset['allowCustom'],
		'supplyKey'     => $preset['supplyKey'],
		'categories'    => $preset['categories'],
		'overrides'     => $preset['overrides'],
	);
	$script = '<script>window.EVREG=' . wp_json_encode( $cfg ) . ';</script>';
	$app = <<<'VOMEVREG_APP'
<style>
...
<header class="hero">
  ...
  <div class="wrap">
    <div class="stats">
      ...
      <div class="stat" id="statItems"><b id="stDishes">0</b><span id="i_stDishes">Items signed up</span></div>
      <div class="stat" id="statCats"><b id="stCats">0</b><span id="i_stCats">Categories covered</span></div>
    </div>
    ...
    <section class="card" id="formCard">
      ...
      <div class="row">
        ...
        <div class="col small" id="partyCol">
...
<script>
  ...
  var CATEGORIES = (window.EVREG && window.EVREG.categories) || [];
  ...
  var TXT = {
    en:{
      ...
      foot:'Your sign-up is saved securely.<br>Made with care by Vision Outreach Media 🌿',
    ...
    nl:{
      ...
      foot:'Je aanmelding wordt veilig opgeslagen.<br>Met zorg gemaakt door Vision Outreach Media 🌿',
  ...
  (function(){ var ov=(window.EVREG&&window.EVREG.overrides)||{}; ['en','nl'].forEach(function(l){ if(ov[l]){ for(var k in ov[l]){ TXT[l][k]=ov[l][k]; } } }); })();
  var EVREG = window.EVREG || {};
  var HAS_PARTY = EVREG.hasParty !== false;
  var HAS_SEL = (EVREG.hasSelections !== false) && CATEGORIES.length > 0;
  var ALLOW_CUSTOM = EVREG.allowCustom !== false;
  var SUPPLYKEY = EVREG.supplyKey || null;
  ...
  var BASKET = [];
  var ACTIVE_TAB = CATEGORIES.length ? CATEGORIES[0].key : '';
  ...
  function renderSuggestions(){
    ...
    CATEGORIES.forEach(function(c){
      ...
      tab.innerHTML='<span>'+c.emoji+'</span>'+((count===0 && c.key!==SUPPLYKEY)?'<span class="dot"></span>':'');
  ...
  function missingCats(){
    return CATEGORIES.filter(function(c){return c.key!==SUPPLYKEY && !(STATE.totals.categories[c.key]>0);});
  }
  function renderNeeded(){
    if(!HAS_SEL){ el('neededWrap').innerHTML=''; return; }
  ...
  function renderList(){
    ...
    if(!HAS_SEL){
      ...
      STATE.items.forEach(function(p){
        var ppl=(HAS_PARTY && p.partySize>0)? p.partySize+' '+esc(t.people):'';
        flat+='<div class="item"><div class="grow"><div class="dish">'+esc(p.name)+'</div>'+((ppl||p.notes)?'<div class="meta">'+ppl+(p.notes?(ppl?' — ':'')+esc(p.notes):'')+'</div>':'')+'</div><button class="rm" aria-label="remove" data-p="'+esc(p.id)+'" data-name="'+esc(p.name)+'">&times;</button></div>';
      });
      host.innerHTML=flat;
      Array.prototype.forEach.call(host.querySelectorAll('.rm'),function(b){ b.onclick=function(){ removeEntry(b.getAttribute('data-p'), null, b.getAttribute('data-name')); }; });
      return;
    }
  ...
  function renderAll(){ renderStats(); renderNeeded(); renderSuggestions(); renderList(); if(HAS_SEL) renderBasket(); updateSubmit(); }
  ...
  function loadState(){ api('state?event='+encodeURIComponent(EVREG.event||'')).then(onState).catch(function(err){ el('list').innerHTML='<div class="empty">'+esc(T().tLoadErr)+'</div>'; console.error(err); }); }

  function submit(){
    if(busy) return;
    var t=T();
    var name=el('name').value.trim();
    var party = HAS_PARTY ? parseInt(el('party').value,10) : 1;
    if(!name){ toast(t.tNeedName); el('name').focus(); return; }
    if(HAS_PARTY && (!party||party<1)){ toast(t.tNeedParty); el('party').focus(); return; }
    if(ALLOW_CUSTOM && el('dish').value.trim()){ addToBasket(ACTIVE_TAB, el('dish').value.trim()); el('dish').value=''; }
    busy=true; var btn=el('submitBtn'); btn.disabled=true; btn.textContent=t.submitting;
    var payload={event:EVREG.event, name:name, partySize:party, notes:el('notes').value.trim(), items:BASKET.slice()};
  ...
  function removeEntry(personId, itemId, name){
    if(busy) return;
    busy=true;
    api('remove',{method:'POST', body:JSON.stringify({event:EVREG.event, personId:personId, itemId:itemId||null})}).then(function(st){ onState(st); busy=false; toast(T().tRemoved(name||'')); })
  ...
  function applyFlags(){
    if(!HAS_PARTY && el('partyCol')) el('partyCol').style.display='none';
    if(!HAS_SEL){
      if(el('bringSection')) el('bringSection').style.display='none';
      if(el('statItems')) el('statItems').style.display='none';
      if(el('statCats')) el('statCats').style.display='none';
    } else if(!ALLOW_CUSTOM){
      if(el('addown')) el('addown').style.display='none';
    }
  }
  applyFlags();
  applyLang();
...
VOMEVREG_APP;
	return '<div class="vom-evreg-app">' . $script . $app . '</div>';
}

/* ---------- admin page + CSV export ---------- */
add_action( 'admin_menu', function () {
	add_menu_page( 'Event Registration', 'Event Registration', 'manage_options', 'vom-evreg', 'vom_evreg_admin_page', 'dashicons-clipboard', 26 );
} );

function vom_evreg_admin_page() {
	$presets = vom_evreg_presets();
	$event   = isset( $_GET['event'] ) ? sanitize_key( wp_unslash( $_GET['event'] ) ) : 'picnic';
	if ( ! isset( $presets[ $event ] ) ) { $event = 'picnic'; }
	$state = vom_evreg_state( $event );
	$t     = $state['totals'];
	echo '<div class="wrap"><h1>Event Registration</h1>';

	// Event switcher.
	echo '<h2 class="nav-tab-wrapper" style="margin-bottom:16px">';
	foreach ( $presets as $key => $p ) {
		$url = admin_url( 'admin.php?page=vom-evreg&event=' . $key );
		$cls = ( $key === $event ) ? ' nav-tab-active' : '';
		echo '<a href="' . esc_url( $url ) . '" class="nav-tab' . $cls . '">' . esc_html( $p['label'] ) . '</a>';
	}
	echo '</h2>';

	// Shortcode + page link.
	echo '<p style="font-size:14px">Shortcode for this form: <code>[event_registration event="' . esc_attr( $event ) . '"]</code>';
	if ( 'picnic' === $event ) {
		$pid = (int) get_option( 'vom_picnic_page_id' );
		if ( $pid && get_post_status( $pid ) === 'publish' ) {
			echo ' &nbsp;&middot;&nbsp; Live page: <a href="' . esc_url( get_permalink( $pid ) ) . '" target="_blank"><strong>' . esc_html( get_permalink( $pid ) ) . '</strong></a>';
		}
	}
	echo '</p>';

	$export = wp_nonce_url( admin_url( 'admin-post.php?action=vom_evreg_export&event=' . $event ), 'vom_evreg_export' );
	...
	echo '<table class="widefat striped"><thead><tr><th>Name</th><th>People</th><th>Selections</th><th>Notes</th></tr></thead><tbody>';
	if ( empty( $state['items'] ) ) {
		echo '<tr><td colspan="4">No sign-ups yet for this form.</td></tr>';
	} else {
		foreach ( $state['items'] as $p ) {
			$sel = array();
			foreach ( $p['items'] as $it ) { $sel[] = esc_html( $it['dish'] ); }
			echo '<tr><td><strong>' . esc_html( $p['name'] ) . '</strong></td><td>' . intval( $p['partySize'] ) . '</td><td>' . ( $sel ? implode( ', ', $sel ) : '&mdash;' ) . '</td><td>' . esc_html( $p['notes'] ) . '</td></tr>';
		}
	}
	echo '</tbody></table></div>';
}

add_action( 'admin_post_vom_evreg_export', function () {
	if ( ! current_user_can( 'manage_options' ) || ! check_admin_referer( 'vom_evreg_export' ) ) { wp_die( 'Not allowed' ); }
	$event = isset( $_GET['event'] ) ? sanitize_key( wp_unslash( $_GET['event'] ) ) : 'picnic';
	$state = vom_evreg_state( $event );
	nocache_headers();
	header( 'Content-Type: text/csv; charset=utf-8' );
	header( 'Content-Disposition: attachment; filename=' . $event . '-signups.csv' );
	$out = fopen( 'php://output', 'w' );
	fputcsv( $out, array( 'Name', 'People', 'Category', 'Selection', 'Notes' ) );
	foreach ( $state['items'] as $p ) {
		if ( empty( $p['items'] ) ) {
			fputcsv( $out, array( $p['name'], $p['partySize'], '', '', $p['notes'] ) );
		} else {
