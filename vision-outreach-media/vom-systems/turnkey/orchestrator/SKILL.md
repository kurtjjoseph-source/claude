...
| See what each forge builds | `$TK archetypes` | CHURCHforge / BIZforge / fallback; no idea folder needed. |
...
**Archetypes.** The spec resolves an archetype from the org type — `church` → **CHURCHforge**
(orange on black), `service_business` / `creator_media` → **BIZforge** (sky blue on black),
everything else → a generic fallback that reports `curated: false`. Both are compositions over
*one* general module-based platform, not separate products: same runtime, same renderer, same
section vocabulary. A new archetype is a data change in `engine/archetypes.py`.
