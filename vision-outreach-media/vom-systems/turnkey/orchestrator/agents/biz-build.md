...
**Check for a Forge build before you commission one.** If the launch has a
`turnkey/platform-spec.json`, the orchestrator can produce the public site *and* the owner's
operating platform in a single unattended run (`turnkey forge "$IDEA" --target local`), built
to this blueprint's archetype and checked against the spec's own assertions. When that output
exists, your job is to **review, correct and place it** — not to rebuild it from scratch:

- read `<idea>/forge/<run-id>/build/` and the run's `console.html`;
- fix what is wrong in the *profile or the archetype*, then re-forge, rather than hand-editing
  generated pages (a hand edit is lost on the next run);
- report the spec's `assumptions` block to the operator — those are the values the builder
  chose because the profile did not supply them, and each one is live copy on a real page.

Commission the dev swarm when the blueprint calls for something the archetype does not cover
— a WordPress build, a custom application, or a function no module implements.
